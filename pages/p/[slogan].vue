<template>
  <main class="pb-16 lg:pb-24 bg-white dark:bg-gray-900">
    <header
      class="bg-[url('{{ post.logo }}')] w-full h-[460px] xl:h-[537px] bg-no-repeat bg-cover bg-center bg-blend-darken relative"
    >
      <div
        class="absolute top-0 left-0 w-full h-full bg-black bg-opacity-50"
      ></div>
      <div
        class="absolute top-20 left-1/2 px-4 mx-auto w-full max-w-screen-2xl -translate-x-1/2 xl:top-1/2 xl:-translate-y-1/2 xl:px-0"
      >
    ...
    <div
      class="flex relative z-20 justify-between p-6 -m-36 mx-4 max-w-screen-2xl bg-white dark:bg-gray-800 rounded xl:-m-32 xl:p-9 xl:mx-auto"
    >
      <article
        class="xl:w-[1080px] w-full max-w-none format format-sm sm:format-base lg:format-lg format-blue dark:format-invert"
      >
      ...
      <aside class="hidden xl:block" aria-labelledby="sidebar-label">
        <div class="xl:w-[336px] sticky top-6">
          ...
          <div>
            ...
            <p
              class="text-xs font-light text-gray-400 uppercase dark:text-gray-500"
            >
              Ads placeholder
            </p>
          </div>
      ...
      <div class="grid gap-6 lg:gap-12 md:grid-cols-2">
        <article class="flex flex-col xl:flex-row">
          ...
          <div class="flex flex-col justify-center">
            ...
            <p
              class="mb-4 font-light text-gray-500 dark:text-gray-400 max-w-sm"
            >
...
onMounted(() => {
  ...
  const tableHeaders = document.querySelectorAll("#aside-item");

  tableHeaders.forEach((header: any) => {
    // Style top of header Height
    // @ts-ignore
    header.style.top = `${mainHeaderHeight + 35}px`;
  });
});
const route = useRoute();
const slogan = route.params.slogan;
const api = usePostsStore();
let post = await api.getPost(slogan);
post = post.data.results;
</script>
